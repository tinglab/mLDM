...
List mLDM(NumericMatrix X, NumericMatrix M, double Z_mean = 1.0, int max_iteration = 2000, double threshold = 1e-4, int approx_num_Z = 10,
  ...
  if(!corX.allFinite() || ! corM.allFinite()) {
    cout << "Initialization Error! NaN found in the spearman correlation matrix among X, or between X and M" << endl;
    return List::create();
  }
  ...
  if(!Z_init.allFinite()) {
    cout << "Initialization Error! NaN found in the Z" << endl;
    return List::create();
  }
  // For B
  MatrixXd B_init = corM.transpose();
  if(!B_init.allFinite()) {
    cout << "Initialization Error! NaN found in the B" << endl;
    return List::create();
  }
  // For Theta
  ...
  if(!Theta_init.allFinite()) {
    cout << "Initialization Error! NaN found in the Theta" << endl;
    return List::create();
  }
  // For B0
  VectorXd B0_init = Z_init.colwise().mean();
  if(!B0_init.allFinite()) {
    cout << "Initialization Error! NaN found in the B0" << endl;
    return List::create();
  }
